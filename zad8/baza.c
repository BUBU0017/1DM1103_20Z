...
Ocena * wczytaj_oceny(FILE *fin)
{
    ...
    for (i=0; i<n; i++)
    {
        ...
        s = strtok(NULL, ";");
        oce->ocena = (char*) malloc( sizeof(char) * (strlen(s) + 1));
        strcpy(oce->ocena, s);
...
SBaza * wczytaj_baze(char *nazwa_pliku)
{
    ...
    if (fin == NULL)
    {
        printf("Nie moge wczytac pliku: %s.\n", nazwa_pliku);
        exit(-1);
    }
...
void listuj_studentow_od_konca(SBaza *l)
{
    Student *k;
    int i;
    int n = ile_studentow(l);
    for (i=(n-1); i >= 0; i--)
    {
        k = zwroc_stud(l->lista_studentow, i);
        printf("%s %s %s %s\n", k->imie, k->nazwisko, k->nr_albumu, k->email);        
    }
}

void listuj_przedmioty_od_konca(SBaza *l)
{
    
    Przedmiot *k;
   
    int i;
    int n = ile_przedmiotow(l);
    for (i=(n-1); i >= 0; i--)
    {
        k = zwroc_przed(l->lista_przedmiotow, i);
        printf("%s %s %s \n", k->kod_przed, k->nazwa_przed, k->sem);        
    }
}
...
Ocena *usun_ocenke(Ocena *glowa, Ocena *el)
{
    Ocena *k = glowa;
    if (glowa == el)
    {
...
Student *sortuj_stud_przez_wybieranie(Student *glowa)
{
    ...
    while (glowa != NULL)
    {
        p = najwiekszy_student(glowa);
        glowa = usun_stud(glowa, p);
        
        q = ostatni_student(nowa_glowa);
        if (q == NULL)
        {
            nowa_glowa = p;
        }
...
Przedmiot *sortuj_przed_przez_wybieranie_kod(Przedmiot *glowa)
{
    ...
    while (glowa != NULL)
    {
        p = najwiekszy_przedmiot_kod(glowa);
        glowa = usun_przedmiot(glowa, p);
        
        q = ostatni_przedmiot(nowa_glowa);
        if (q == NULL)
        {
            nowa_glowa = p;
        }
...
Przedmiot *sortuj_przed_przez_wybieranie_nazwa(Przedmiot *glowa)
{
    ...
    while (glowa != NULL)
    {
        p = najwieksza_przedmiot_nazwa(glowa);
        glowa = usun_przedmiot(glowa, p);
...
Ocena *filtruj_przedmiot(Ocena *glowa, char *kod, SBaza *baza)
{
    ...
    for (int i = 0; i < ile_ocen(baza); i++)
    {
        ...
        { 
            ...
            {
                p = ostatnia_ocena(k);
                ostatnia_ocena(k)->nast = nowaglowa;
                nowaglowa->poprz = p;
            }
...
Ocena *filtruj_ucznia(Ocena *glowa, char *album, SBaza *baza)
{
    Ocena *k = NULL;
    Ocena *p = NULL;
